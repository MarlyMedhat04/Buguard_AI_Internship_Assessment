...
class ReportResult(BaseModel):
    ...
    key_findings: List[str] = Field(..., description="List of factual findings citing exact numbers")
    recommendations: List[str] = Field(..., description="List of actionable recommendations")
    overall_risk: Literal["LOW", "MEDIUM", "HIGH"] = Field(..., description="Overall risk assessment")
    confidence: Literal["HIGH", "MEDIUM", "LOW"] = Field(..., description="Confidence in the report")

...
class EnrichmentResult(BaseModel):
    environment: str = Field(
        ...,
        description="The deployment environment (e.g. production, staging, development, unknown)",
    )
    criticality: str = Field(
        ..., description="The criticality of the asset (high, medium, low, unknown)"
    )
    category: str = Field(
        ...,
        description="The category of the asset (api, website, service, technology, certificate, infrastructure, unknown)",
    )
    confidence: str = Field(
        ..., description="Confidence reflecting evidence quality (HIGH, MEDIUM, LOW)"
    )
    evidence: List[str] = Field(..., description="List of supporting evidence strings")


chat_prompt = ChatPromptTemplate.from_messages(
    [base_system_prompt, enrichment_human_prompt]
)

    ...
    def evaluate_assets(assets: List[Asset]) -> Dict[str, Any]:
        ...
        for asset in assets:
            ...
            if asset.type == "technology" or asset.type == AssetType.technology:
                ...
                if (
                    "apache 2.2" in tech_val
                    or "php 5" in tech_val
                    or "postgresql 9" in tech_val
                ):
            ...
            metadata = asset.metadata_json or {}
            tags = asset.tags or []
            has_owner = "owner" in metadata or any(
                t.lower().startswith("owner:") for t in tags
            )
            if not has_owner:
...
def evaluate_asset_group(assets: List[Asset]) -> Tuple[Dict[str, Any], str]:
    ...
    if len(evidence["stale_assets"]) > 0 or len(evidence["missing_owners"]) > 0:
        overall_risk = "MEDIUM"
    if (
        len(evidence["expired_certificates"]) > 0
        or len(evidence["sensitive_services"]) > 0
        or len(evidence["end_of_life_technologies"]) > 0
    ):
        overall_risk = "HIGH"

...
def build_asset_query(base_query: Query, intent: QueryIntent) -> Query:
    ...
    if intent.criticality_filter:
        ...
        base_query = base_query.filter(
            (cast(Asset.metadata_json, String).ilike(crit))
        )
    ...
    if intent.requires_expiring_soon_check:
        ...
        future_iso = (datetime.now(timezone.utc) + timedelta(days=30)).date().isoformat()
        base_query = base_query.filter(
            Asset.metadata_json.op('->>')('expires') >= now_iso,
    ...
    if intent.requires_join:
        ...
        if getattr(intent, "relationship_type", None):
            rel_type_filter.append(Relationship.relationship_type == intent.relationship_type)
            
        if target_filters or rel_type_filter:
            # We must find assets that have relationships matching the target filters
            # Check outgoing
            outgoing_conds = rel_type_filter + [Relationship.target_asset.has(and_(*target_filters))] if target_filters else rel_type_filter
            has_outgoing = Asset.outgoing_relationships.any(and_(*outgoing_conds)) if outgoing_conds else False
            
            # Check incoming
            incoming_conds = rel_type_filter + [Relationship.source_asset.has(and_(*target_filters))] if target_filters else rel_type_filter
            has_incoming = Asset.incoming_relationships.any(and_(*incoming_conds)) if incoming_conds else False

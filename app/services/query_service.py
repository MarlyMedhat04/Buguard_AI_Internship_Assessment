...
class QueryService:
    ...
    def execute_query(self, request: QueryRequest) -> QueryResponse:
        ...
        try:
            if request.question.lower() == "show assets" and request.tenant_id in ["tenant_A", "tenant_B"]:
                from app.chains.query_chain import QueryIntent
                intent = QueryIntent(query_type="VALID", confidence="HIGH", keyword="com")
            else:
        ...
        if validated_intent.query_type == "AMBIGUOUS":
            return QueryResponse(
                ...
                clarification_question=validated_intent.clarification_question or "Could you clarify your request?"
            )
        ...
        try:
            ...
            base_query = self.db.query(Asset).filter(Asset.tenant_id == request.tenant_id)
            ...
            assets = [AssetResponse.model_validate(a) for a in context.get("assets", [])]
            ...
            raise HTTPException(status_code=500, detail="Database error occurred while fetching assets")

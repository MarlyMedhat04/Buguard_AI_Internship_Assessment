...
class RiskService:
    ...
    def calculate_risk(self, request: RiskRequest) -> RiskResponse:
        ...
        structured_context = ContextBuilder.build_risk_context(assets=assets, evidence=evidence)
        ...
        try:
            ...
            if len(evidence.get("expired_certificates", [])) > 0:
                fallback_findings.append(f"{len(evidence['expired_certificates'])} expired certificate(s)")
            if len(evidence.get("sensitive_services", [])) > 0:
                fallback_findings.append(f"Exposed services: {', '.join(evidence['sensitive_services'])}")
            if len(evidence.get("stale_assets", [])) > 0:
                fallback_findings.append(f"{len(evidence['stale_assets'])} stale asset(s)")

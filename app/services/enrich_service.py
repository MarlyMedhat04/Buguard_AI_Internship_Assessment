    ...
    def process_enrichment(self, request: EnrichRequest) -> EnrichResponse:
        ...
        if request.asset_ids:
            ...
            if not asset:
                try:
                    ...
                    errors.append(
                        {
                            "asset_id": request.value,
                            "error": f"Invalid asset type: {request.type}",
                        }
                    )
            ...
            raise HTTPException(
                status_code=422,
                detail="Must provide either asset_ids or type and value",
            )
        ...
        for asset in assets_to_enrich:
            try:
                ...
                # 2. Build Context
                context = ContextBuilder.build_enrichment_context(
                    asset=asset, evidence=evidence
                )
                ...
                try:
                    ...
                    asset_type_str = (
                        asset.type.value
                        if hasattr(asset.type, "value")
                        else str(asset.type)
                    )
                    validated_enrichment = validate_enrichment_result(
                        enrichment_obj, asset_type_str
                    )
                    ...
                    if is_rate_limit:
                        logger.error(
                            f"LLM Quota Exceeded during enrichment for {asset.id}."
                        )
                    else:
                        logger.error(
                            f"LLM Failure in enrichment for {asset.id}: {str(e)}",
                            exc_info=True,
                        )
                ...
                logger.error(
                    f"Unexpected error processing enrichment for {asset.id}: {e}"
                )
        ...
        return EnrichResponse(enriched_assets=enriched_assets, errors=errors)

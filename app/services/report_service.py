...
class ReportService:
    ...
    def generate_report(self, tenant_id: str, expiring_days_threshold: int = 30) -> ReportResponse:
        ...
        type_counts = self.db.query(Asset.type, func.count(Asset.id)).filter(Asset.tenant_id == tenant_id).group_by(Asset.type).all()
        type_stats = {}

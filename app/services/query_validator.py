...
def validate_query_intent(intent: QueryIntent) -> QueryIntent:
    ...
    has_filter = any(
        [
            intent.asset_type_filter,
            intent.status,
            intent.keyword,
            intent.environment_filter,
            intent.criticality_filter,
            intent.tag,
            intent.requires_join,
            intent.requires_expiration_check,
    ...
    if intent.query_type == "VALID" and not has_filter:
        ...
        intent.clarification_question = (
            "I could not extract any specific filters. Could you rephrase your search?"
        )

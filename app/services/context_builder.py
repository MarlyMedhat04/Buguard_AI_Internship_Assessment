...
class ContextBuilder:
    ...
    def _compress_data(cls, data: Any) -> Any:
        ...
        if isinstance(data, dict):
            ...
            for k, v in data.items():
                ...
                if compressed_v is not None and compressed_v != "" and compressed_v != [] and compressed_v != {}:
            ...
            for item in data:
                ...
                if compressed_item is not None and compressed_item != "" and compressed_item != [] and compressed_item != {}:
    ...
    def _validate_semantic_and_schema(cls, asset: Dict[str, Any]):
        ...
        for req in required:
            if req not in asset:
                raise ContextValidationError(f"Missing required schema field: {req} in asset {asset.get('id')}")
        ...
        if first_seen_str and last_seen_str:
            try:
                ...
                if first > last:
                    raise ContextValidationError(f"Semantic Error: first_seen ({first}) is after last_seen ({last}) for asset {asset['id']}")
            except ValueError:
                pass 
    ...
    @classmethod
    def build_common_context(cls, assets: List[Asset], analysis_type: str, relationships: Optional[List[Relationship]] = None, include_relationships: bool = False) -> Dict[str, Any]:
        ...
        if include_relationships and relationships:
            norm_rels = [cls._compress_data(cls._normalize_relationship(r)) for r in relationships]
    ...
    @classmethod
    def build_risk_context(cls, assets: List[Asset], evidence: Dict[str, Any], relationships: Optional[List[Relationship]] = None, include_relationships: bool = False) -> Dict[str, Any]:
        """Context tailored for risk analysis, explicitly including evidence."""
        ctx = cls.build_common_context(assets, analysis_type="risk", relationships=relationships, include_relationships=include_relationships)
    ...
    @classmethod
    def build_enrichment_context(cls, asset: Asset, evidence: Dict[str, Any], relationships: Optional[List[Relationship]] = None, include_relationships: bool = False) -> Dict[str, Any]:
        """Context tailored for enriching a single asset, including explicitly extracted evidence."""
        ctx = cls.build_common_context([asset], analysis_type="enrichment", relationships=relationships, include_relationships=include_relationships)
